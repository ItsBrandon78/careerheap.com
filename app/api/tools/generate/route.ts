...
async function runTool(tool: string, body: GenerateBody) {
  if (tool === 'resume-analyzer') {
    if (!body.resumeText || body.resumeText.trim().length < 30) {
      return { error: 'Paste at least a few lines of your résumé to analyze.' as string }
    }
    return generateResumeAnalysis({ resumeText: body.resumeText, targetRole: body.targetRole })
  }
  if (tool === 'interview-prep') {
    if (!body.role || !body.role.trim()) {
      return { error: 'Enter a target role to generate questions.' }
    }
    return generateInterviewPrep({ role: body.role, context: body.context })
  }
  if (tool === 'cover-letter') {
    if (!body.jobPosting || body.jobPosting.trim().length < 30) {
      return { error: 'Paste the job posting so we can tailor the letter.' }
    }
...
export async function POST(request: NextRequest) {
  ...
  if (planOverride) {
    ...
    const outcome = await runTool(tool, body)
    if ('error' in outcome) {
