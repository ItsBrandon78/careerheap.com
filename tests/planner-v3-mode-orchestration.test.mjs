...
test('dashboard component includes required control actions and stale warning text', () => {
  ...
  assert.match(dashboardSource, /What to do first/)
  assert.match(dashboardSource, /Your path forward/)
  ...
  assert.match(dashboardSource, /If this path stalls, try one of these/)
  assert.doesNotMatch(dashboardSource, /Alternative Career Paths/)
...
test('intake component keeps 3-step flow controls and step-3 generation gating', () => {
  // 3-step progress chrome: the prototype Stepper maps over wizardSteps using activeStep.
  assert.match(intakeSource, /<Stepper steps=\{wizardSteps\} activeStep=\{activeWizardStep\}/)
